...
int window; 
long currentParticles = NUM_PARTICLES;

// Inicializando todos os pixels
void InitAllParticles(int pause){
    int i;
    currentParticles = NUM_PARTICLES;

    if(pause) usleep(20000 + rand() % 200000);

    for(i=0;i<NUM_PARTICLES;i++) {

        float velocity = 0.00000000000001;
        int angle = rand() % 360;
        particles[i].velocidadeX = cos((3.14 * angle/180.0)) * velocity;
        particles[i].velocidadeY = fabs(sin((3.14 * angle/180.0)) * velocity/100);
        particles[i].x = 0;
        particles[i].y = 2.25;
        particles[i].lifetime = 80;
    }
}
...
void InitParticle(int index){
    ...
    int angle = rand() % 360;
    particles[index].velocidadeX = cos((3.14 * angle/180.0)) * velocity;
    particles[index].velocidadeY = fabs(sin((3.14 * angle/180.0)) * velocity);
    particles[index].x = 0;
...
void DrawGLScene(){
    ...
    glBegin(GL_POINTS); 
    for(i=0;i<currentParticles;i++){ // Inicializando as particulas no topo da tela
